...
const StrainSearch = () => {
  ...
  const addToRequest = (strain: StrainItem) => {
      ...
      <div className="sticky top-0 bg-background/95 backdrop-blur-sm border-b border-border z-30">
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="relative mb-4">
            ...
            <Input placeholder="Search strain, product, type, or effect..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-12 py-3 text-lg bg-card border-border text-foreground focus:border-primary" />
          </div>

          <div className="flex gap-2 overflow-x-auto pb-2">
            {filters.map(filter => (
              <Button key={filter} variant={selectedFilter === filter ? "default" : "outline"} size="sm" onClick={() => setSelectedFilter(filter)}
                className={`whitespace-nowrap ${selectedFilter === filter ? 'bg-primary text-primary-foreground' : 'border-border hover:bg-primary/10'}`}>
                {filter}
              </Button>
            ))}
          </div>
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <Card key={strain.id} className="bg-card border-border hover:border-primary/50 transition-all duration-300 group hover:scale-105 hover:shadow-lg hover:shadow-primary/25">
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start mb-2">
                  <CardTitle className="text-foreground group-hover:text-primary transition-colors text-lg">{strain.name}</CardTitle>
                  <Badge className={getTypeColor(strain.type)}>{strain.type}</Badge>
                </div>
                {strain.thc && <span className="text-sm text-muted-foreground">THC: {strain.thc}</span>}
              </CardHeader>
              <CardContent className="space-y-3">
                ...
                <p className="text-muted-foreground text-sm">{strain.description}</p>
                {strain.flavor && <p className="text-primary text-sm"><span className="font-medium">Flavor:</span> {strain.flavor}</p>}
                ...
                <div className="text-primary font-semibold text-lg">KSh {strain.price.toLocaleString()}</div>
              </CardContent>
              <CardFooter>
                <Button onClick={() => addToRequest(strain)} disabled={selectedStrains.some(s => s.id === strain.id)}
        ...
        <div className="mt-16 pt-8 border-t border-border">
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 text-center">
            <p className="text-sm text-muted-foreground">⚠️ For adults 21+ only. Licensed dispensary. Pickup only. No online sales.</p>
          </div>
        </div>
      </div>
      ...
      {selectedStrains.length > 0 && (
        <div className="fixed bottom-6 right-6 z-40">
          <Button onClick={() => setShowRequestModal(true)}
            className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg hover:shadow-xl hover:shadow-primary/25 transition-all duration-200 pl-4 pr-6 py-3 rounded-full" size="lg">
